...
export KERNEL_CONFIG="bindroid_n1awifi_defconfig"
...
GETVER=`grep 'BinDroid-.*-V' .config | sed 's/.*".//g' | sed 's/-S.*//g'`
...
./mkbootimg --kernel ${KERNELDIR}/arch/arm/boot/zImage --ramdisk $INITRAMFS_TMP.cpio.gz --board universal5420 --base 0x10000000 --pagesize 2048 --ramdiskaddr 0x11000000 -o ${KERNELDIR}/boot.img.pre
...
cp ${KERNELDIR}/.config ${KERNELDIR}/arch/arm/configs/${KERNEL_CONFIG}_P600
cp ${KERNELDIR}/.config ${KERNELDIR}/READY/
rm ${KERNELDIR}/READY/boot/zImage
rm ${KERNELDIR}/READY/Kernel_*
stat ${KERNELDIR}/boot.img
cp ${KERNELDIR}/boot.img /${KERNELDIR}/READY/
cd ${KERNELDIR}/READY/
        zip -r Kernel_Bindroid-`date +"[%H-%M]-[%d-%m]-SM-P600"`.zip .
rm ${KERNELDIR}/boot.img
rm ${KERNELDIR}/READY/boot.img
rm ${KERNELDIR}/READY/.config
        ...
        if [ "$ADB_STATUS" == "device" ]; then
			...
			mv ${KERNELDIR}/READY/Kernel_* ${KERNELDIR}/P600/
			ncftpput -f /home/dev/login.cfg -V -R / ${KERNELDIR}/P600/
			rm ${KERNELDIR}/P600/Kernel_*
